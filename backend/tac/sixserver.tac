...
service = TCPServer(scfg.WebInterface['port'], registrationServer, 
    interface='0.0.0.0')  # Listen on all interfaces for Docker networking
service.setServiceParent(application)
...
reactor.listenTCP(adminConfig.AdminPort, adminServer, interface=config.interface)
...
statsRoot.putChild(
    b'my-account', admin.UserAccountResource(adminConfig, config))
statsRoot.putChild(b'ps', admin.ProcessInfoResource(adminConfig, config, False))
statsRoot.putChild(b'announcements', admin.PublicAnnouncementsResource(adminConfig, config))
# Match history endpoint
matchesRoot = admin.MatchHistoryResource(adminConfig, config)
statsRoot.putChild(b'matches', matchesRoot)
matchesRoot.putChild(b'history', matchesRoot)
statsServer = Site(statsRoot)
statsService = TCPServer(adminConfig.AdminPort+1, statsServer, 
    interface='0.0.0.0') # allow external requests
